...
- **Agent 主动互调**：Structured Agent 可通过 GroupX MCP 使用 `send`、`publish`、`ask`、`collect` 和 `read`；`publish` 公开但不唤醒，pending ask 只按原消息精确 collect。同一 correlation 中已在运行的 Agent 用 `publish/read` 继续当下讨论，只有明确需要后续独立 Turn 时才 `send/ask`，避免意见已被当前回合吸收后又延迟重放。GroupX 不指定协调者、互评拓扑或讨论轮数，由 Agent 按任务自行组织；监督 Watch Turn 另有 `watch` 与 `steer`。
